...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = qkv[0], qkv[1], qkv[2] # [B, nHeads, nPatch, nDim]

        attn_unnorm = (q @ k.transpose(-2, -1)) * self.scale # [B, nHeads, nQ, nK]
        attn = attn_unnorm.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, attn, attn_unnorm
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False, return_all=False):
        y, attn, attn_unnorm = self.attn(self.norm1(x))
        if return_attention:
            return attn
        x = x + self.drop_path(y)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        if return_all:
            return x, attn_unnorm
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, num_cls_tokens=1, pretrained=None,
                 **kwargs):
        super().__init__()
        ...
        self.cls_token = nn.Parameter(torch.zeros(1, num_cls_tokens, embed_dim))
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + num_cls_tokens, embed_dim))
        ...
        # self.attn2seg = Atten2Seg(num_cls_tokens, embed_dim, num_heads)

        trunc_normal_(self.pos_embed, std=.02)
        trunc_normal_(self.cls_token, std=.02)
        self.apply(self._init_weights)

        self.init_params(pretrained)
    ...
    def init_params(self, pretrained=None):
        ...
        # pos_embed = self.resize_pos_embed(pos_embed, grid_old_shape=(14, 14), grid_new_shape=(32, 32), num_extra_tokens=1)
    ...
    def interpolate_pos_encoding(self, x, w, h):
        npatch = x.shape[1] - self.num_cls_tokens
        N = self.pos_embed.shape[1] - self.num_cls_tokens
        if npatch == N and w == h:
            return self.pos_embed
        class_pos_embed = self.pos_embed[:, :self.num_cls_tokens]
        patch_pos_embed = self.pos_embed[:, self.num_cls_tokens:]
        dim = x.shape[-1]
    ...
    def prepare_tokens(self, x):
        B, nc, w, h = x.shape
        x = self.patch_embed(x)  # patch linear embedding, [B, nPatch, dim]
    ...
    def forward_train(self, img, targets=None):
        ...
        x = self.prepare_tokens(img) # [B, nPatch, nDim]
        assert x.shape[1] == nh * nw + self.num_cls_tokens, (img.shape, x.shape, (nh*nw, nh, nw))

        for blk in self.blocks[:-1]:
            x = blk(x)
        x, attn = self.blocks[-1](x, return_all=True)
        x = self.norm(x)
        # [B, C], [B, C, H, W]
        
        return {
            "attention_mask": attn[:, :, :self.num_cls_tokens, self.num_cls_tokens:],
            "feature": x,
        }
